...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Safety fallback: force-show all reveals and trigger counters/bars after 900ms
  // in case IntersectionObserver doesn't fire (GitHub Pages iframe, some mobile browsers)
  setTimeout(() => {
    $$(".reveal").forEach((el) => el.classList.add("is-visible"));
    $$(".bar > i").forEach((el) => { if (el.dataset.bar) el.style.width = el.dataset.bar + "%"; });
    $$(".progress-fill").forEach((el) => { if (el.dataset.progress) el.style.width = el.dataset.progress + "%"; });
    $("[data-count]") && $$(  "[data-count]").forEach((el) => {
      const raw = el.textContent.trim();
      if (raw === "0" || raw === "0%" || raw === "0M AED") animateCounter(el);
    });
    const jt = $("#journeyTrack"); if (jt) jt.classList.add("is-drawn");
    const wl = $("#waqfLifecycle"); if (wl) wl.classList.add("is-drawn");
  }, 900);
});
